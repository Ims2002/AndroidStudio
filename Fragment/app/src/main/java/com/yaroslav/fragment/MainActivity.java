...
public class MainActivity extends AppCompatActivity implements IContactListener, FragmentOnAttachListener {
    ...
    public void onSelectedContact(Contact c) {
        //If is tablet
        if(tabletLayout) {
            frgDetail.showDetail(c);
            //c.getName(), c.getSurname1(), c.getSurname2(), c.getBirth(), c.getCompany(), c.getEmail(), c.getPhone1(), c.getPhone2(), c.adress
        }
        //If is phone
        else {
            Intent i = new Intent(this, DetalleActivity.class);
            i.putExtra(DetalleActivity.EXTRA_CONTACT,c);
    ...
    public void onAttachFragment(@NonNull FragmentManager fragmentManager, @NonNull Fragment fragment) {
        if(fragment.getId() == R.id.FrgListado) {
            ...
            if(tabletLayout) {
                ...
                bundle.putSerializable(FragmentDetalle.CONTACT_CONTACT,fragmentListado.getDatas()[0]);
                /*
